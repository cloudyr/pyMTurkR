#' Disable HIT
#' 
#' Disabling a HIT is probably not what you want to do. DisableHIT
#' automatically removes the HIT from the MTurk server, approves (and thus pays
#' for) all submitted and pending assignments, and then permanently deletes all
#' assignment data.
#' 
#' Disable a HIT (and its assignment data). This is a somewhat risky function
#' because it automatically approves all pending assignments and then disposes
#' of everything. \code{\link{DisposeHIT}} is probably what most users will use
#' to delete HIT and assignment data that is no longer needed.
#' 
...
#' @references
#' \href{http://docs.amazonwebservices.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_DisableHITOperation.htmlAPI
#' Reference}
#' @keywords HITs
#' @examples
#' 
