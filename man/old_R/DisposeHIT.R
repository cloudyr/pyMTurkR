#' Dispose HIT
#' 
#' Dispose of a HIT that is no longer needed. You can only dispose of HITs that
#' are Reviewable, with all assignments either approved or rejected.
#' 
#' Dispose of a HIT (and its assignment data) when it is no longer needed. Must
#' specify a HITId or a HITTypeId, but not both. HITTypeId uses the
#' \code{\link{SearchHITs}} operation to locate HITs of the specified HITType
...
#' dispose of all HITs from a \dQuote{batch} created in the online Requester
#' User Interface (RUI). To use a batch ID, the batch must be written in a
#' character string of the form \dQuote{BatchId:78382;}, where \dQuote{73832}
#' is the batch ID shown in the RUI. Must specify \code{hit} xor
#' \code{hit.type} xor \code{annotation}.
#' @param response.group An optional character string specifying what details
#' of each HIT to return of: \dQuote{Minimal}, \dQuote{HITQuestion},
#' \dQuote{HITDetail}, \dQuote{HITAssignmentSummary}. For more information, see
...
#' dispose of each of them was valid.
...
#' @references
#' \href{http://docs.amazonwebservices.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_DisposeHITOperation.htmlAPI
#' Reference}
#' @keywords HITs
#' @examples
#' 
