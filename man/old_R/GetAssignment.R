...
#' 
#' \code{GetAssignments()}, \code{assignment()}, and \code{assignments()} are
#' aliases.
#' 
...
#' \code{assignment} xor \code{hit} xor \code{hit.type} xor \code{annotation}.
#' @param status An optional character string (of
...
#' @param return.all If \code{TRUE}, all available assignments are returned.
#' Otherwise, only assignments falling within the specified \code{pagenumber}
#' and \code{pagesize} search results are returned.
#' @param pagenumber An optional character string indicating which page of
#' search results should be returned (only appropriate when specifying a single
#' HITId). Most users can ignore this.
#' @param pagesize An optional character string indicating how many search
#' results should be returned by each request (only appropriate when specifying
#' a single HITId), between 1 and 100. Most users can ignore this.
#' @param sortproperty One of \dQuote{AcceptTime}, \dQuote{SubmitTime},
#' \dQuote{AssignmentStatus}. Ignored if \code{return.all=TRUE}. Most users can
#' ignore this.
...
#' strings) specifying what details to return. If \code{assignment} is
#' specified, \code{response.group} can include any of \dQuote{Request},
#' \dQuote{Minimal}, \dQuote{AssignmentFeedback}, \dQuote{HITDetail}, and/or
#' \dQuote{HITQuestion}. If \code{hit} or \code{hit.type} is specified,
#' \code{response.group} can include \dQuote{Request}, \dQuote{Minimal}, and/or
#' \dQuote{AssignmentFeedback}. For more information, see
#' \href{http://docs.aws.amazon.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_CommonParametersArticle.htmlCommon
#' Parameters}.
#' @param return.assignment.dataframe A logical specifying whether the
#' Assignment data frame should be returned. Default is \code{TRUE}.
...
#' @references
#' \href{http://docs.amazonwebservices.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_GetAssignmentOperation.htmlAPI
#' Reference: GetAssignment}
#' 
#' \href{http://docs.amazonwebservices.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_GetAssignmentsForHITOperation.htmlAPI
#' Reference: GetAssignmentsForHIT }
